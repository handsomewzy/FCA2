...
from .basicvsr_arch import ResidualBlocksWithInputConv, SPyNet, BasicVSRNet_CAV, BasicVSRNet
from .ftvsr_arch import FTVSR
from .basicvsr_plusplus_arch import BasicVSRPlusPlusNet
...
class FCA2(BaseModule):
    ...
    def __init__(self,
        ...
        # 先加载 .pth 文件内容
        # load_net = torch.load('/data1/userhome/luwen/Code/wzy/CAD2VSR/experiments/vid4_gae_93/models/net_g_32000.pth') # 旧的 ftvsr用的 还没有颜色修正
        load_net = torch.load('/data1/userhome/luwen/Code/wzy/CAD2VSR/experiments/vid4_gae93_color/models/net_g_190000.pth') # vimeo
        # load_net = torch.load('/data1/userhome/luwen/Code/wzy/CAD2VSR/experiments/REDS_gae93_color/models/net_g_5000.pth')
        ...
        # 和其他别的网络进行级联
        # self.add_network = FTVSR()
        # self.add_network = BasicVSRNet()
        self.add_network = BasicVSRPlusPlusNet()
        
        # 先加载 .pth 文件内容
        # load_net = torch.load('/data1/userhome/luwen/Code/wzy/CAD2VSR/experiments/vid4_ftvsr/models/net_g_360000.pth')
        # load_net = torch.load('/data1/userhome/luwen/Code/wzy/CAD2VSR/experiments/vid4_basicvsr/models/net_g_110000.pth')
        load_net = torch.load('/data1/userhome/luwen/Code/wzy/CAD2VSR/experiments/vid4_basicvsr++/models/net_g_220000.pth')
        
        # load_net = load_net["params"]
        # self.add_network.load_state_dict(load_net)
    ...
    def forward(self, lqs, return_lqs=False):
        ...
        for i in range(len(lqs_sublist)):
            ...
            _outputs = self.add_network(lqs_sublist[i])
