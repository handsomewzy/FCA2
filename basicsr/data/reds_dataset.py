...
class REDSRecurrentDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print(type(self.lq_root), self.lq_root)
        ...
        for neighbor in neighbor_list:
            if self.is_lmdb:
                ...
                # 获取sub_id对应的lq_subroot值
                lq_sub = self.lq_subroot[sub_id]
                # 构建初始路径
                img_lq_path = self.lq_root / clip_name / f'{neighbor:08d}.png'
                # 如果lq_sub不是'BD'，则替换路径中的'train_sharp_bicubic'
                if lq_sub != 'BD':
                    # 将路径中的'train_sharp_bicubic'替换为相应的后缀
                    img_lq_path = str(img_lq_path).replace('train_sharp_bicubic', f'train_sharp_bicubic_{lq_sub}')
